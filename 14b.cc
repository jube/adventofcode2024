...
namespace {
  ...
  void display(const std::vector<Input>& inputs)
  {
    std::vector<std::string> screen(Height, std::string(Width, ' '));

    for (auto& input : inputs) {
      screen[input.py][input.px] = '*';
    }
  ...
  bool is_a_tree(const std::vector<Input>& inputs)
  {
    std::vector<std::string> screen(Height, std::string(Width, ' '));
    ...
    for (auto& line : screen) {
      if (line.find(std::string(Threshold, '*')) != std::string::npos) {
        return true;
      }
    }
